...
>
> Fast suite (~52s, 46 tests): 21 files. Slow suite (~6.5min, 2 tests): 2 files.
...
- **Hash fragility**: MD5 hashes of RDS output files (`tools::md5sum`) are
  fragile — they break on R serialization format changes, package version bumps,
  and numerical drift. For regression tests that guard against a specific
  algorithmic change, prefer comparing a single numeric value from the output
  against a threshold at the midpoint of the two versions:
  1. Run both versions (old and fixed) to obtain V1 and V2.
  2. Set threshold at midpoint `(V1+V2)/2`, rounded to nearest 1,000.
  3. Assert the output is on the correct side of the threshold.
  Example: `sum(UDPolygons$area)` or `max(UDPolygons$area)` discriminate
  between two smoothing parameters (`mag` vs `scaleARS`) because they produce
  materially different polygon areas (~44% difference). The midpoint provides a
  ~15% safety margin against numerical drift.
