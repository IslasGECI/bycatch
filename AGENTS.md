...
>
> Fast suite (~52s, 42 tests): 19 files. Slow suite (~6.5min, 4 tests): 2 files.
...
- **Red phase**: format tests, run. If they fail → stage `tests/testthat/*.R` → commit. If pass → `git restore .`
- **Green phase**: format R/, run. If pass → stage `R/*.R` → commit. If fail → `git restore .`
- **Refactor phase**: format both, run. If pass → stage all → commit. If fail → `git restore .`
...
- **`R/`** — 6 files: `cli.R` (Level 2: create_*, render_*), `compute.R` (all compute_*), `plot.R` (internal plot_*), `io.R` (import_config, import_returning_trips), `get_domain_specific_options.R` (parser flags).
- **`tests/testthat/`** — 19 fast + 2 slow in `slow/`. One `describe()` per file. Paths hardcoded as `/workdir/tests/data/…` (Docker convention).
