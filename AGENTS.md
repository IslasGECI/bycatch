...
- **Mocking sf functions in tests**: Use `testthat::local_mocked_bindings()` with `.package = "sf"` to verify that internal functions like `sf::st_make_valid` are called. Save the original before mocking to avoid infinite recursion:
  ```r
  original_fn <- sf::some_function
  called <- FALSE
  local_mocked_bindings(
    some_function = function(x) { called <<- TRUE; original_fn(x) },
    .package = "sf"
  )
  ```
