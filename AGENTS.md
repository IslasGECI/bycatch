...
> Fast tests (~22s) can be run with `make tests_fast`, which skips the slow render tests
...
- **`tests/testthat/`** — 9 test files (8 fast, 1 directory of slow tests). Uses `testthat` edition 3 + `testtools` helpers for file-existence assertions.
  - `test_compute.R` — tests for standalone `compute_*` functions.
  - `slow/test_cli_slow.R` — slow render tests, excluded from `make tests_fast`.
